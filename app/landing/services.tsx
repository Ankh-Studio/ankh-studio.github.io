...
const ServiceBackground = () => {
  const maskId = `cutout-mask-${Math.random()}`;
  const blurBorderFilterId = `blur-border-filter-${Math.random()}`
  const dotBorderFilterId  = `dot-border-filter-${Math.random()}`
  ...
  useEffect(() => {
    setRadius(size.height/2 - glowBorder/2 - dotBorder/2);
  }, [size]);
...
const ServiceCard = ({
  ...
  return (
    <div 
      className={cn(
        ...
        activeCard === id ? 'z-10' : 'z-0'
      )}
      onClick={() => setActiveCard(
        id === activeCard ? null : id
      )}
    >
      <div className={cn(
        "relative overflow-hidden rounded-2xl backdrop-blur-xl border border-white/20",
        "bg-gradient-to-br from-black/40 via-purple-900/20 to-blue-900/40",
        "shadow-2xl transition-all duration-500 ease-out",
        "hover:scale-105 hover:shadow-purple-500/25 hover:border-purple-500/30",
  ...
  //    )}>
  //      <h3 className={cn(
  //        "text-xl font-semibold mb-3 group-hover:text-purple-400 transition-colors",
  //        //"2xl:text-xl 2xl:mx-auto 2xl:mt-4"
  //      )}>
  //        {service.title}
  //      </h3>
  //      <p className={cn(
  //        "text-gray-400",
  //        //"2xl:mb-8 2xl:mx-6 2xl:text-xl 2xl:text-center"
  //      )}>{service.summary}</p>
  //    </div>
  //  </div>
  //);
...
const Services = () => {
  ...
  return (
    <section id="services" className="service_container relative min-h-screen py-20 px-6 ">
      <ServiceBackground />
      <div className={cn(
